...
def create_test_user(session: TestSession) -> Tuple[bool, str]:
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/users", json={"username": f"test_user_{int(time.time())}"}
        )
...
def test_list_user_agents(session: TestSession) -> bool:
    ...
    try:
        response = requests.get(f"{BASE_URL}/users/me/agents", headers=session.headers)
...
def test_agent_operations(session: TestSession, agent_id: UUID) -> bool:
    ...
    try:
        ...
        metrics_response = requests.get(
            f"{BASE_URL}/agent/{agent_id}/metrics", headers=session.headers
        )
...
def test_completion(session: TestSession, agent_id: UUID) -> bool:
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/agent/completions", headers=session.headers, json=payload
        )
    ...
    for agent_id in session.test_agents:
        try:
            response = requests.delete(
                f"{BASE_URL}/agent/{agent_id}", headers=session.headers
            )
            if response.status_code == 200:
    ...
    if session.user_id:
        try:
            response = requests.get(
                f"{BASE_URL}/users/{session.user_id}/api-keys", headers=session.headers
            )
            if response.status_code == 200:
